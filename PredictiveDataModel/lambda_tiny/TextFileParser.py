...
class TextFileParser:
    ...
    def _convert_types(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['season'] = pd.to_numeric(df['season'], errors='coerce').astype('Int64')  # Nullable integer
        df['week'] = pd.to_numeric(df['week'], errors='coerce').astype('Int64')  # Nullable integer
        ...
        # Convert spread_line - NO FLIP NEEDED
        # Format: positive = home favored, negative = away favored
        df['spread_line'] = pd.to_numeric(df['spread_line'], errors='coerce')
        
        df['total_line'] = pd.to_numeric(df['total_line'], errors='coerce')
        
        # Handle div_game - convert to boolean, treating NaN as False
        df['div_game'] = pd.to_numeric(df['div_game'], errors='coerce').fillna(0).astype(int).astype(bool)
