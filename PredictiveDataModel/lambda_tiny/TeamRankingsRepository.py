...
class TeamRankingsRepository:
    ...
    def upsert_rankings(self, rankings_df: pd.DataFrame) -> int:
        ...
        update_columns = [col for col in columns if col not in ['team_id', 'season']]
        update_set = ', '.join([f"{col} = EXCLUDED.{col}" for col in update_columns])
        
        query = f"""
            INSERT INTO team_rankings ({columns_str})
            VALUES ({placeholders})
        ...
        for _, row in rankings_df.iterrows():
            try:
                ...
                for col in columns:
                    ...
                    # Integer columns
                    if col in ['season', 'games_played', 'wins', 'losses', 'ties',
                              'total_points_scored', 'total_points_allowed', 'point_differential',
                              'offensive_rank', 'defensive_rank', 'overall_rank',
                              'home_games', 'home_wins', 'home_losses',
                              'away_games', 'away_wins', 'away_losses',
                              'div_games', 'div_wins', 'div_losses',
                              'times_favored', 'times_underdog',
                              'ats_wins', 'ats_losses', 'ats_pushes'] and value is not None:
                        params[col] = int(float(value))
                    # Decimal/Float columns
                    elif col in ['win_rate', 'avg_points_scored', 'avg_points_allowed',
                                'avg_point_differential', 'home_win_rate',
                                'home_avg_points_scored', 'home_avg_points_allowed',
                                'away_win_rate', 'away_avg_points_scored', 'away_avg_points_allowed',
                                'div_win_rate', 'avg_spread_line', 'avg_total_line',
                                'ats_cover_rate', 'avg_spread_margin'] and value is not None:
                ...
                logger.error(f"Error upserting rankings for {row.get('team_id', 'unknown')} {row.get('season', 'unknown')}: {e}")
                continue
