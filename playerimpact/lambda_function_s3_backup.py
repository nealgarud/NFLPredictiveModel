...
def lambda_handler(event, context):
    ...
    try:
        ...
        if season not in _cached_madden_ratings:
            ...
            # DEBUG: Log Madden data info
            if not madden_data.empty:
                logger.info(f"Loaded {len(madden_data)} players from Madden CSV")
                logger.info(f"Madden CSV columns: {list(madden_data.columns)}")
                sample_row = madden_data.iloc[0]
                logger.info(f"Sample Madden player: name={sample_row.get('player_name')}, id={sample_row.get('player_id')}, rating={sample_row.get('overallrating')}")
            else:
                logger.warning("Madden data is EMPTY!")
        else:
...
def _calculate_team_impact_from_roster(
    ...
    if len(players) > 0:
        ...
        logger.info(f"Sample player keys: {list(sample.keys())[:10]}")
